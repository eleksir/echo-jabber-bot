...
func parseEvent(e interface{}) {
	...
	case xmpp.Chat:
		...
		if v.Text != "" {
			...
			// Групповой чятик
			case "groupchat":
				log.Debugf("Message from public chat")
				...
				log.Debugf("Trying to echo message back to chat %s", dest)
				...
				if _, err := talk.Send(
					...
					log.Errorf("Unable to send message to %s: %s", v.Remote, err)
			...
			case "chat":
				...
				log.Debug("Definitely not public chat")
				log.Trace(spew.Sdump(e))

				if config.NoEcho {
					return
				}

				log.Debugf("Trying to echo message back to chat %s", v.Remote)
	...
	case xmpp.IQ:
		...
		case xmpp.IQTypeGet:
			...
			if err := xml.Unmarshal(v.Query, &iqStruct); err == nil {
				...
				case "jabber:iq:version":
					...
					if id, err := talk.RawInformation(
						...
						v.ID,
						xmpp.IQTypeResult,
						"<query xmlns=\"jabber:iq:version\"><name>echo-jabber-bot</name><version>4.с_чем-то</version><os>линупс</os></query>",
					); err != nil {
			...
			// Хотя идейно мы не поддерживаем работу без севрерной оптимизации, но на пинг ответим, нам не сложно.
			if parseError {
		...
		case xmpp.IQTypeSet:
			...
			log.Info("Got an IQ reqest for set something. Answer not implemented")
				...
				if err := xml.Unmarshal(v.Query, &iqErrorCancelNotAcceptable); err == nil {
					if v.To == talk.JID() {
						...
						if slices.Contains(roomsConnected, iqErrorCancelNotAcceptable.By) && nick == config.Jabber.Nick {
							...
							log.Error(
								"looks like message parsed as jabberIqErrorCancelNotAccepatble but we're not in given room",
							)
						...
						log.Error(
							"Looks like message parsed as jabberIqErrorCancelNotAccepatble but addressed not to us",
						)
		...
		default:
			log.Info("Got an unknown IQ reqest. Dunno how deal with it, discarding")
			log.Info(spew.Sdump(e))
		}
	...
	case xmpp.Presence:
		...
		if v.Type == xmpp.IQTypeError {
			...
			log.Infof("Presence notification, Type: %s, From: %s, To: %s Show: %s, Status: %s",
				v.Type, v.From, v.To, v.Show, v.Status)
